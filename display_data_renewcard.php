...
$query ="SELECT * FROM renew_card ORDER BY ID DESC";
...
<html>
	...
	<script type="text/javascript" src="https://cdn.datatables.net/v/bs5/jq-3.6.0/jszip-2.5.0/dt-1.12.1/af-2.4.0/b-2.2.3/b-colvis-2.2.3/b-html5-2.2.3/b-print-2.2.3/cr-1.5.6/date-1.1.2/fc-4.1.0/fh-3.2.4/kt-2.7.0/r-2.3.0/rg-1.2.0/rr-1.2.8/sc-2.0.7/sb-1.3.4/sp-2.0.2/sl-1.4.0/sr-1.1.1/datatables.min.js"></script>
			...
			<?php
				while($row = mysqli_fetch_array($result))
				{
					...
					<tr>
						...
						<td>' .$row["duration"]. '</td>
						<td>' .$row["balance"]. '</td>
						<td><a class= "btn btn-primary" href="edit_renewcard.php?edit='.$row["id"].'">Update</a></td>
						<td><a class= "btn btn-danger" href="display_data_renewcard.php?del='.$row["id"].'">Delete</a></td>
					</tr>
					';
				}
			?>
		...
		<td><a class= "btn btn-success" href="add_renewcard.php">Add New Card </a></td>
		</div>
		</body>
...
if(isset ($_GET['del'])){
	$del_id = $_GET['del'];
	$delete = "DELETE FROM renew_card WHERE id='$del_id'";
